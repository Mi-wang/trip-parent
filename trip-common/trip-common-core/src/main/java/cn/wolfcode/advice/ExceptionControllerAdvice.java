...
public class ExceptionControllerAdvice {
    ...
    public R<?> commonExceptionHandler(RuntimeException ex) {
        log.error("[统一异常处理] 出现系统异常", ex);

        return R.err(HttpStatus.SERVER_ERROR, ex.getMessage());
    }

    @ExceptionHandler(Wolf2wException.class)
    public R<?> commonExceptionHandler(Wolf2wException ex) {
        log.error("[统一异常处理] 服务内部异常", ex);

        return R.err(ex.getCode(), ex.getMessage());
    }
    ...
    public R<?> validExceptionHandler(BindException ex) {
        ...
        return R.err(HttpStatus.BAD_REQUEST, error.getDefaultMessage());
    }
    ...
    public R<?> validExceptionHandler(ConstraintViolationException ex) {
        ...
        return R.err(HttpStatus.BAD_REQUEST, msg);
    }
}
